...
var index = 0;
$(function () {
    /* 这段代码的作用是为 class 为 "nav" 的元素添加一个点击事件处理函数，
    当用户点击其中一个元素时，该元素会被突出显示，
    并且其它元素的突出显示状态将被取消。
    同时，该函数还会记录被点击元素在其兄弟元素中的位置，以供后续操作使用。 */
    // $('.nav').click(function () {
    //     type = $(this).index();
    ...
    /* 为 class 为 "btn" 的元素添加一个点击事件处理函数，
    在用户点击其中一个按钮时，它会将该按钮高亮显示，
    并取消其它按钮和其它 .arithmeticList 元素下的按钮的高亮状态，
    并修改页面中对应的链接地址。*/
    // $('.btn').click(function () {
    //     index = $(this).index();
